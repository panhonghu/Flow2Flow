...
for idx, (img, label, cam, path, item) in enumerate(train_loader):
    ...
    rgb2ir, _ = FlowModel.g_ir(rgb2lat, reverse=True)
    rgb2ir = torch.tanh(rgb2ir)
    # generate rgb
    ir2lat, _ = FlowModel.g_ir(FlowModel.ir, reverse=False)
    ir2rgb, _ = FlowModel.g_rgb(ir2lat, reverse=True)
    ir2rgb = torch.tanh(ir2rgb)
    ...
    rgb2ir, ir2rgb = un_normalize(rgb2ir).detach().cpu(), un_normalize(ir2rgb).detach().cpu()
...
# for idx, (img, label, cam, path, item) in enumerate(gallery_loader):
#     print('gallery loader idx ', idx)
#     path_rgb = path
#     img_rgb = img.cuda()
#     FlowModel.set_inputs(img_rgb, img_rgb, label, label)
#     # generate ir
#     rgb2lat, _ = FlowModel.g_rgb(FlowModel.rgb, reverse=False)
#     rgb2ir, _ = FlowModel.g_ir(rgb2lat, reverse=True)
#     rgb2ir = torch.tanh(rgb2ir)
#     img_rgb = un_normalize(img_rgb).detach().cpu()
#     rgb2ir = un_normalize(rgb2ir).detach().cpu()
...
# #### 6 save query images
# for idx, (img, label, cam, path, item) in enumerate(query_loader):
#     print('query loader idx ', idx)
#     path_ir = path
#     img_ir = img.cuda()
#     FlowModel.set_inputs(img_ir, img_ir, label, label)
#     # generate ir
#     ir2lat, _ = FlowModel.g_ir(FlowModel.ir, reverse=False)
#     ir2rgb, _ = FlowModel.g_rgb(ir2lat, reverse=True)
#     ir2rgb = torch.tanh(ir2rgb)
#     img_ir = un_normalize(img_ir).detach().cpu()
#     ir2rgb = un_normalize(ir2rgb).detach().cpu()
